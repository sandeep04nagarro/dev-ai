...
async def get_dashboard_metrics(login: str) -> dict[str, Any]:
    ...
    for t in threads:
        ...
        if status == "running":
            ...
            status == "finished"
            or status == "completed"
            or (status == "idle" and not getattr(t, "error", None))
        ):
            completed_threads += 1

            # Avg Completion Time
            created_at = (
                t.get("created_at") if isinstance(t, dict) else getattr(t, "created_at", None)
            )
            updated_at = (
                t.get("updated_at") if isinstance(t, dict) else getattr(t, "updated_at", None)
            )
        ...
        if metadata.get("jira_issue_key"):
            source = "jira"
        elif metadata.get("slack_channel_id"):
            source = "slack"
        elif metadata.get("github_issue"):
            source = "github"
        elif metadata.get("linear_issue"):
            source = "linear"
    ...
    return {
        "overview": {
            ...
            "average_completion_time_seconds": (total_completion_seconds / completed_with_duration)
...
async def list_dashboard_sandboxes(login: str) -> list[dict[str, Any]]:
    ...
    for t in threads:
        ...
        if sandbox_id not in sandboxes:
            ...
            full_name = "unknown"
            if (
                "selected_repos" in metadata
                and isinstance(metadata["selected_repos"], list)
                and len(metadata["selected_repos"]) > 0
            ):
