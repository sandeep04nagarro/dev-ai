...
from agent.utils.multi_repo_registry import _get_fallback_repos
from agent.utils.repo import extract_repos_from_text


def test_extract_repos_from_text():
    # Single repo fallback
    repos = extract_repos_from_text(
        "Please fix this in repo: langchain-ai/langchain", "default-owner"
    )
    assert len(repos) == 1
    ...
    # Multiple repos
    repos = extract_repos_from_text(
        "Please fix this. repos: langchain-ai/backend, other-owner/frontend", "default-owner"
    )
    assert len(repos) == 2
